-- 清空现有数据（可选，用于测试环境）
-- TRUNCATE TABLE users, drivers, driver_reviews, vehicles, vehicle_reviews RESTART IDENTITY CASCADE;

-- 插入测试用户数据
INSERT INTO app_user (role_id, nick_name, open_id, session_key, union_id, last_login, created_at, updated_at) VALUES
...
-- 插入测试司机数据（已审核通过的司机）
INSERT INTO driver (open_id, license_number, name, phone, license_image_url, status, created_at, updated_at) VALUES
...
-- 插入待审核司机数据
INSERT INTO driver_review (open_id, license_number, name, phone, license_image_url, status, comment, action_type, driver_id, created_at, updated_at) VALUES
...
-- 插入测试车辆数据（已审核通过的车辆）
INSERT INTO vehicle (driver_id, plate_number, vehicle_type, brand, model_name, color, year, registration_image, insurance_expiry, status, comment, submit_time, created_at, updated_at) VALUES
...
-- 插入待审核车辆数据
INSERT INTO vehicle_review (driver_id, plate_number, vehicle_type, brand, model_name, color, year, registration_image, insurance_expiry, status, comment, action_type, vehicle_id, created_at, updated_at) VALUES
...
-- 验证数据插入
SELECT 'Users' as table_name, COUNT(*) as count FROM app_user
UNION ALL
SELECT 'Drivers' as table_name, COUNT(*) as count FROM driver
UNION ALL
SELECT 'Driver Reviews' as table_name, COUNT(*) as count FROM driver_review
UNION ALL
SELECT 'Vehicles' as table_name, COUNT(*) as count FROM vehicle
UNION ALL
SELECT 'Vehicle Reviews' as table_name, COUNT(*) as count FROM vehicle_review;