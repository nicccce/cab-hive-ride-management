...
func (m *ModuleDriver) InitRouter(r *gin.RouterGroup) {
	...
	r.POST("/auth/driver/self-update", middleware.Auth(1), DriverSelfUpdateHandler)
	
	// 获取所有司机列表 - 需要管理员认证
	// 接口地址: GET /api/users/drivers
	r.GET("/users/drivers", middleware.Auth(3), GetAllDrivers)
	
	// 获取司机信息 - 需要用户认证
	// 接口地址: GET /api/users/drivers/:id
	r.GET("/users/drivers/:id", middleware.Auth(1), GetDriver)
	...
	// 获取司机自己的所有司机审核信息列表 - 需要司机认证
	// 接口地址: GET /api/users/drivers/self/pending
	r.GET("/users/drivers/self/pending", middleware.Auth(2), GetSelfPendingDrivers)
}