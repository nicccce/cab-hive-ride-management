...
// PhoneLastDigitsResponse 定义手机尾号响应的结构体
type PhoneLastDigitsResponse struct {
	PhoneLastDigits string `json:"phone_last_digits"` // 用户手机尾号倒数第4第3位
}
...
// GetOrderUserPhoneLastDigits 处理司机获取订单用户手机尾号的请求
// 该接口返回司机名下未完结订单的用户手机尾号倒数第4第3位
func GetOrderUserPhoneLastDigits(c *gin.Context) {
	// 从上下文中获取载荷
	payloadInterface, exists := c.Get("payload")
	if !exists {
		response.Fail(c, response.ErrTokenInvalid)
